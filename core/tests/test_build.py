...
def test_build_started():
    build_event = Build(build_type=BuildType.BuildStartedEventV1, id="_id", name="_name", artifact="_artifact", data={"key1": "value1"})
...
def test_build_queued():
    build_event = Build(build_type=BuildType.BuildQueuedEventV1, id="_id", name="_name", artifact="_artifact", data={"key1": "value1"})
    assert build_event is not None
...
def test_build_finished():
    build_event = Build(build_type=BuildType.BuildFinishedEventV1, id="_id", name="_name", artifact="_artifact", data={"key1": "value1"})
    assert build_event is not None
