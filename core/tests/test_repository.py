...
def test_repository_created():
    repository_event = Repository(repository_type=RepositoryType.RepositoryCreatedEventV1, id="_id", name="_name", url="_url", data={"key1": "value1"})
...
def test_repository_modified():
    repository_event = Repository(repository_type=RepositoryType.RepositoryModifiedEventV1, id="_id", name="_name", url="_url", data={"key1": "value1"})
    assert repository_event is not None
...
def test_repository_deleted():
    repository_event = Repository(repository_type=RepositoryType.RepositoryDeletedEventV1, id="_id", name="_name", url="_url", data={"key1": "value1"})
    assert repository_event is not None
