...
def test_environment_modified():
    env_event = EnvEvent(env_type=EnvType.EnvironmentModifiedEventV1, id="_id", name="_name", repo="_repo", data={"key1": "value1"})
    assert env_event is not None
...
def test_environment_deleted():
    env_event = EnvEvent(env_type=EnvType.EnvironmentDeletedEventV1, id="_id", name="_name", repo="_repo", data={"key1": "value1"})
    assert env_event is not None
